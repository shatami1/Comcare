...
function buildAdminMailto(subject, body) {
    ...
    return `mailto:admin@comcare.store?${params.toString()}`;
}
...
function generateCartEmailLink() {
    ...
    const lines = ['Discount Request for MOON LIGHTING INC. DBA Comfort Care Cart:', ''];
    if (!cartItems.length) {
        lines.push('No items in cart.');
    } else {
        cartItems.forEach(item => {
            lines.push(`Item: ${item.name || item.equipment || ''}, Model: ${item.model || ''}, Rate: ${item.rateType || item.rate || ''}, Quantity: ${item.quantity || 1}, Price: $${item.unitPrice || item.price || 0}`);
        });
        lines.push('', `Total: $${getStoredCheckoutTotal()}`);
    }
    const subject = 'MOON LIGHTING INC. DBA Comfort Care Cart Discount Request';
    const mailto = buildAdminMailto(subject, lines.join('\n'));
...
function showCheckoutReturnNotice() {
    ...
    const isSubscriptionSuccess = checkoutStatus === 'subscription-success';
    if (checkoutStatus !== 'success' && !isSubscriptionSuccess) {
        return;
    }
    ...
    if (isSubscriptionSuccess) {
        localStorage.removeItem('comfortCareRecoveryPackage');
        notice.innerHTML = `
            <strong>Monthly recovery package started.</strong>
            <span>Square will email a receipt and manage the recurring monthly billing. Comfort Care will contact you to confirm package details and delivery needs.</span>
        `;
    } else {
        notice.innerHTML = `
...
const RECOVERY_PACKAGE_CART_ITEMS = {
    essential: {
...
function setRecoveryPackageCheckoutMessage(message, state = 'info') {
    const messageEl = document.getElementById('recoveryPackageCheckoutMessage');
    if (!messageEl) {
        return;
    }

    messageEl.textContent = message;
    messageEl.classList.remove('is-visible', 'is-info', 'is-error');

    if (message) {
        messageEl.classList.add('is-visible', state === 'error' ? 'is-error' : 'is-info');
    }
}

function createRecoveryPackageCartItem(packageCode, packageInfo) {
    return {
        name: `${packageInfo.name} Support Package`,
        model: 'Recovery support package - first month charge',
        rateType: 'One-time package charge',
        unitPrice: packageInfo.price,
        quantity: 1,
        offerCode: 'RECOVERY_SUPPORT_PACKAGE',
        packageCode,
        manualRenewal: true
    };
}

function bindRecoveryPackageCheckoutButtons() {
    const packageButtons = document.querySelectorAll('.recovery-package-checkout[data-package-code]');
    if (!packageButtons.length) {
        return;
    }

    packageButtons.forEach(button => {
        if (button.dataset.packageCartBound === 'true') {
            return;
        }

        button.dataset.packageCartBound = 'true';
        button.addEventListener('click', function(event) {
            event.preventDefault();

            const packageCode = String(this.dataset.packageCode || '').trim().toLowerCase();
            const packageInfo = RECOVERY_PACKAGE_CART_ITEMS[packageCode];
            ...
            const cartWithoutRecoveryPackages = currentCart.filter(item => item.offerCode !== 'RECOVERY_SUPPORT_PACKAGE');
            const nextCart = [
                ...cartWithoutRecoveryPackages,
                createRecoveryPackageCartItem(packageCode, packageInfo)
            ];

            localStorage.setItem('pricingCartItems', JSON.stringify(nextCart));
            saveCheckoutSnapshot(nextCart);
            localStorage.setItem('comfortCareRecoveryPackage', JSON.stringify({
                packageCode,
                name: packageInfo.name,
                price: packageInfo.price,
                cadence: 'monthly',
                billingMode: 'manual-renewal'
            }));

            updateHeaderCart();
            updateCartEmailLink();
            this.textContent = 'Added - opening cart...';
            setRecoveryPackageCheckoutMessage(`${packageInfo.name} was added to the cart as a first-month package charge. Future monthly renewals can be handled manually by Comfort Care.`, 'info');
            window.setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', bindRecoveryPackageCheckoutButtons);
...
function updateCartEmailLink() {
    ...
    const cart = getStoredCart();
    if (cart.length === 0) {
        link.href = buildAdminMailto('MOON LIGHTING INC. DBA Comfort Care Discount Request', 'My cart is currently empty.');
        return;
    }
    ...
    link.href = buildAdminMailto(subject, body);
}
...
function initPricingCart() {
    ...
    function renderCart() {
        ...
        cart.forEach((item, index) => {
            ...
            const displayRate = rateLabels[item.rateType] || item.rateType || 'Item';
            meta.textContent = `${item.model} | ${displayRate} | Qty ${item.quantity}`;
...
function initCheckoutPage() {
    ...
    if (checkoutEmailLink) {
        ...
        checkoutEmailLink.href = buildAdminMailto(subject, body);
    }
